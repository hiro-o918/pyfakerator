...
def user_schema_record(
    *,
    ...
    return {
        ...
        "name": f.Unset.unwrap_or_else(
            name, lambda: f.gen_string(min_length=5, max_length=10, seed=seed_)
        ),
        "email": f.Unset.unwrap_or_else(
            email, lambda: f.gen_string(min_length=5, max_length=10, seed=seed_)
        ),
